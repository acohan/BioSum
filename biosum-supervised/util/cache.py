...
import gzip
import sys
from util.common import (mkdir_p, hash_obj)
...
class CacheOperator(object):
    def __init__(self, cachedir, prefix=None, mute=False):
        self.cachedir = cachedir
        self._prefix = ('%s_' % prefix) if prefix else ''
        self.mute = mute
    ...
    def get_cache(self, label):
        cc = '%s%s' % (self._prefix, hash_obj(label))
        cache_path = os.path.join(self.cachedir, 'co_%s.cache.gz' % cc)
        if os.path.exists(cache_path):
            with gzip.open(cache_path) as cf:
                return json.loads(cf.read())
        else:
            return None

    def set_cache(self, label, obj):
        cc = '%s%s' % (self._prefix, hash_obj(label))
        return self._co(obj, cache_comment=cc, mute=self.mute)
